...
public class JSONUtils {
    ...
    public static ArrayList<Trailers> getTrailersFromJSON(JSONObject jsonObject){
        ...
        try {
            ...
            for(int i= 0; i<jsonArray.length(); i++){
                JSONObject jsonObjectReview = jsonArray.getJSONObject(i);
                String key = BASE_YOUTUBE_URL + jsonObjectReview.getString(KEY_OF_VIDEO);
                String name = jsonObjectReview.getString(KEY_NAME);
                Trailers review = new Trailers(key, name);
                result.add(review);
            }
        } catch (JSONException e) {
            e.printStackTrace();
